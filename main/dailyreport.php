...
<div class="container-fluid">
	...
	<div class="contentheader">
			...
			<ul class="breadcrumb">
			<li class="active icon-bar-chart" style="font-size: 25px;"> Daily Sales Report</li>
			</ul>
<br>
...
<center><strong><input style="width: 123px; height:35px; margin-top:-8px;margin-left:8px;"  name="daily" type="date" value = "" autocomplete="off"/ />
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	$date2 = date("y/m/d", strtotime($date1));
		$query="SELECT * FROM sales WHERE date='$date2'";
		...
		$count=mysqli_num_rows($result);
	{
		...
		{	$total_amount_query = "SELECT SUM(amount) AS total_amount FROM sales WHERE DATE(date) = '$date2'";
			...
			$total_profit_query = "SELECT SUM(profit) AS profit FROM sales WHERE DATE(date) = '$date2'";
			...
			$total_profit = $total_profit_row['profit'];
				...
				{
				$name = $row['name'] ? $row['name'] : "Walk-in Customer"; // check if the name is empty
			?>
				<tr class="record">
		...
		{
			echo '<tr><td colspan="6" style = "text-align:center;">No daily sales today!!</td></tr>';
		}
	...
	<thead>
		<tr>
			...
			<?php echo isset($total_amount) ? number_format($total_amount, 2) : "0.00"; ?>
				...
				<th colspan="1" style="border-top:1px solid #999999"> ₱
				<?php echo isset($total_profit) ? number_format($total_profit, 2) : "0.00"; ?>
				</th>
		</tr>
	</thead>
</table>
