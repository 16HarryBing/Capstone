...
<div class="container-fluid">
	...
	<div class="contentheader">
			...
			<ul class="breadcrumb">
			<li class="active icon-bar-chart" style="font-size: 25px;"> Daily Inventory Report</li>
			</ul>
<br>
...
<center><strong>From : <input style="width: 223px; height:35px; color:#222;" name="dailyinv" type="date"  value="" autocomplete="off" / />
...
{
    $date=$_POST['dailyinv'];
    $result = $db->prepare("SELECT date, name, SUM(qty) as qty_sold, SUM(amount) as earnings FROM sales_order WHERE date = :date GROUP BY date, name ORDER BY date DESC, name ASC");
...
?>
    <tr class="record">
    <?php if($i==0){ // show the date only once ?>
        <td rowspan="<?php echo $result->rowCount(); ?>"><?php echo date('F j, Y', strtotime($row['date'])); ?></td>
        <?php } ?>
