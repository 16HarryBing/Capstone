...
<title>
POS
</title><link rel="shortcut icon" href="images/background.jpg">
...
<form action="salesreport.php" method="get">
	<center>
		<strong>
			From: <input type="date" style="width: 223px; height:35px; color:#222;" name="d1" value="" autocomplete="off"/> 
			To: <input type="date" style="width: 223px; height:35px; color:#222;" name="d2" class="" value="" autocomplete="off"/> 
...
// Convert dates to the format stored in the database
$d1 = date('Y-m-d', strtotime($_GET['d1']));
$d2 = date('Y-m-d', strtotime($_GET['d2']));

$result = $db->prepare("SELECT date, SUM(amount) as total_amount, SUM(profit) as total_profit FROM sales WHERE date BETWEEN :a AND :b GROUP BY date");
...
<tr class="record">
	...
	<td>₱ <?php echo formatMoney($row['total_profit'], true); ?></td>
</tr>
<?php } ?>
</tbody>
